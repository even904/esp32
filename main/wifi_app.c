...
static void wifi_event_handler(void *arg, esp_event_base_t event_base,
  ...
  if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_START) {
    ...
    s_retry_num = 0;
    // 异步初始化SNTP，确保不会阻塞主任务
    xTaskCreatePinnedToCore(sntp_initialize_task, "sntp_init", 2048, NULL, 5,
                            NULL, tskNO_AFFINITY);
    // 等待一段时间确保SNTP同步完成
...
extern const char
    amap_com_root_cert_pem_start[] asm("_binary_amap_com_root_cert_pem_start");
extern const char
    amap_com_root_cert_pem_end[] asm("_binary_amap_com_root_cert_pem_end");
...
void client_get_weather(char *city_code, extensions_type extensions) {
  ...
  int len =
      snprintf(url_str, sizeof(url_str),
               ...
               city_code, AMAP_API_KEY, extensions != all ? "base" : "all");
  ...
  esp_http_client_config_t config = {
      .url = url_str,
      .cert_pem =
          amap_com_root_cert_pem_start, // 如果需要自定义CA证书，请提供其路径
  ...
  // esp_err_t conn_err;
  // if ((conn_err = esp_http_client_open(client, 0)) != ESP_OK) {
